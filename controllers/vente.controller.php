...
// Table_cmd
try {
    $Cmde = ModeleClasse::getoneByname('Statut_commande', 'table_commande', 100);
    // deb($Cmde);
    $ALL_FACTURE_EN_COURS = ModeleClasse::getallJoinclose('table_commande', 'Numero_commande', 'table_vente', 'Numero_commande', 'Statut_commande', 100);
...
// deb($_GET);
    try {
        //code...
        $Client_selectionnez = ModeleClasse::getOneJoinclose('table_vente', 'ID_client', 'table_client', 'id', 'Statut_vente', 100);
        // deb($Client_selectionnez);
    } catch (\Throwable $th) {
        //throw $th;
        return $error = $th->getMessage();
    }
...
if (!empty($_POST)) :
    ...
    if (isset($_POST['new-commande'])) :
        ...
        try {
            $Cmde = ModeleClasse::getoneByname_DESC('table_commande');
            // deb($Cmde);
            if (!empty($Cmde)) :
                // deb($Cmde);
                ...
                extract($_POST);
                // deb($_POST);
                ModeleClasse::add('table_vente', $_POST);
                ...
                extract($_POST);
                // deb($_POST);
                ModeleClasse::add('table_commande', $_POST);
                header('Location:' . LINK . 'vente');
            else :
                // deb(1);
    ...
    elseif (isset($_POST['add-product'])) :
        ...
        try {
            //code...
            $Commande = ModeleClasse::getallbyName('table_commande', 'Statut_commande', 100);
            // deb($Commande);
            $Verif = [];
    ...
    elseif (isset($_POST['Remise'])) :
        ...
        try {
            //code...
            ModeleClasse::updateClause('table_vente', 'Remise', $Remise, 'Statut_vente', 100);
            header('Location:' . LINK . 'vente');
        } catch (\Throwable $th) {
            return $error = $th->getMessage();
            //throw $th;
    ...
    elseif (isset($_POST['select-client'])) :
        ...
        try {
            ...
            header('Location:' . LINK . 'vente/' . $_GET['id']);
        } catch (\Throwable $th) {
            return $error = $th->getMessage();
            //throw $th;
        }
    elseif (isset($_POST['valider-cmd'])) :
        ...
        try {
            ...
            extract($_POST);
            // deb($_POST);
            ModeleClasse::update('table_vente', $_POST, $Id_vente);
            // unset($_POST);
            ModeleClasse::updateClause('table_vente', 'Statut_vente', 200, 'Numero_commande', $Numero_commande);
            ModeleClasse::updateClause('table_commande', 'Statut_commande', 200, 'Numero_commande', $Numero_commande);
            // deb(1);
            $list_cmd = ModeleClasse::getallByname('table_commande', 'Numero_commande', $Numero_commande);
            unset($_POST);
            // deb($list_cmd);
            foreach ($list_cmd as $data) :
                if (!empty($data['Ref_article'])) :
                    ...
                    extract($_POST);
                    $Art = ModeleClasse::getoneByname('id', 'table_produit', $id);
                    // deb($_POST);
                    $Quantite_produit_last = $Art['Quantite_produit'];
                    $_POST['Quantite_produit'] = $Quantite_produit_last - $data['Quantite_vendu'];
                    ModeleClasse::update('table_produit', $_POST, $Art[0]);
                endif;
            endforeach;
            // deb($list_cmd);
            header('Location:' . LINK . 'vente');
    ...
    elseif (isset($_POST['creance'])) :
        ...
        // deb($_POST);
        if (empty($ID_client)) :
            return $warning = "Veillez choisir / Enregistrer un client pour <span class='text-danger'>CREDIRE</span>";
        endif;
    ...
    elseif (isset($_POST['id_retrait_article'])) :
        ...
        try {
            //code...
            ModeleClasse::delete($id_retrait_article, 'table_commande');
            header('Location:' . LINK . 'vente');
        } catch (\Throwable $th) {
            return $error = $th->getMessage();
            //throw $th;
    ...
    elseif (isset($_POST['annuler_cmd'])) :
        ...
        try {
            //code...
            ModeleClasse::delete_clause('table_commande', 'Numero_commande', $annuler_cmd);
            ModeleClasse::delete_clause('table_vente', 'Numero_commande', $annuler_cmd);
            header('Location:' . LINK . 'vente');
        } catch (\Throwable $th) {
            return $error = $th->getMessage();
            //throw $th;
